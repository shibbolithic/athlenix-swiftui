...
class ViewUserViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
                ...
                do {
                    ...
                    DispatchQueue.main.async {
                        LoadingIndicator.shared.hide()
                                    ...
                                    self.showErrorAlert(message: "Failed to load data. Please try again.")
                                }
                }
            }
        }
    ...
    private func hideContentUntilLoaded(_ hide: Bool) {
        let alpha: CGFloat = hide ? 0.0 : 1.0
        
        // Add all your UI elements that should be hidden/shown
        let contentViews: [UIView] = [
            profilePicture, backgroundPicture, userName, playerOrCoachLabel,
            userBio, position, matches, height, weight, ppg, bpg, ast,
            teamCollectionView, tableView, bestGameMyTeamLogo, bestGameMyTeamName,
            bestGameOpponentTeamLogo, bestGameOpponentTeamName,
            bestGameMyTeam2pters, bestGameMyTeam3pters, bestGameMyTeamFreeThrows,
            bestGameOpponentTeam2pters, bestGameOpponentTeam3pters, bestGameOpponentFreeThrows
        ]
        
        UIView.animate(withDuration: 0.3) {
            contentViews.forEach { $0.alpha = alpha }
        }
    }
...
//            ast.text = "\(athleteProfile.averageAssistsPerGame)"
            ...
            if let sessionUserID = await SessionManager.shared.getSessionUser() {
                let matchesPlayed = await calculateGamesPlayedsupabase(forUserID: selectedUserID!)
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if abs(difference) < scrollThreshold {
            return
        }
        
        // Always show navigation bar at the top of the content
        if currentOffset <= 0 {
        ...
        if difference > 0 && !isNavigationBarHidden {
            // Scrolling down - hide navigation bar
            navigationController?.setNavigationBarHidden(true, animated: true)
            isNavigationBarHidden = true
        } else if difference < 0 && isNavigationBarHidden {
