...
//    @IBOutlet weak var ast: UILabel!
    ...
    var coachProfile: CoachProfileTable?
    var teams1: [Teams] = []
    var bestGame: Game?
...
//                    reloadUserData()// Re-fetch updated user profile data
    ...
    func setupPrimaryDataSupabase(forUserID userID: UUID) async {
        do {
            ...
            else if user11?.role == .coach{
                let athleteResponse = try await supabase.from("CoachProfile").select("*").eq("coachID", value: userID).single().execute()
                let athleteDecoder = JSONDecoder()
                let matchedAthleteProfile = try athleteDecoder.decode(CoachProfileTable.self, from: athleteResponse.data)
                coachProfile = matchedAthleteProfile
            }
        } catch {
    ...
    func setupProfileDetailsSupabase() async {
        ...
        if user11.role == .coach, let coachProfile1 = coachProfile {
            if let sessionUserID = await SessionManager.shared.getSessionUser() {
                let matchesPlayed = await calculateGamesPlayedsupabase(forUserID: sessionUserID)
                matches.text = "\(matchesPlayed)"
                yearsOfExperience.text = "\(coachProfile1.yearsOfExperience)"
            } else {
        ...
        Task {
           ...
           let createdAtString = post.createdAt
            ...
            if let createdDate = dateFormatter.date(from: createdAtString) {
                ...
                DispatchQueue.main.async {
                    cell.createdAt.text = relativeTime
                    LoadingIndicator.shared.hide()
                }
            } else {
                DispatchQueue.main.async {
