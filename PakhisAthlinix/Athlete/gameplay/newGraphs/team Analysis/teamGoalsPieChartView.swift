...
class teamGoalsPieChartView: UIView {
    ...
    private func loadDataFromSupabase() async {
        // Replace with the actual team ID you want to fetch data for
        ...
        do {
            ...
            // Aggregate the data for all players in the team
            let totalFreeThrowsMade = gameLogs.reduce(0) { $0 + $1.freeThrows }
            let totalFreeThrowsMissed = gameLogs.reduce(0) { $0 + ($1.freeThrows - $1.freeThrows) } // Assuming all freeThrows were attempted
            ...
            print("Updated Percentages: \(percentages)")
...
class teamGoalsPieChartContainerView: UIView {
    private var pieChartView: GoalsPieChartView!
    private var legendView: teamLegendView!
